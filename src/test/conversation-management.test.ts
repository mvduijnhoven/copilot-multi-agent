...
export async function runConversationManagementTests(): Promise<void> {
  ...
  const coordinatorContext = TestDataFactory.createCoordinatorContext();
  mockAgentEngine.setMockContext('coord-123', coordinatorContext);
  ...
  setTimeout(() => {
    ...
    if (activeConvs.length > 0) {
      const context = TestDataFactory.createDelegatedContext({ conversationId: activeConvs[0].conversationId });
      mockAgentEngine.setMockContext(activeConvs[0].conversationId, context);
  ...
  setTimeout(() => {
    ...
    if (firstConv) {
      ...
      const context = TestDataFactory.createDelegatedContext({ conversationId: firstConv.conversationId });
      mockAgentEngine.setMockContext(firstConv.conversationId, context);
  ...
  setTimeout(() => {
    ...
    if (remainingConv) {
      // Set the context in mock engine so reportOut can find it
      const context = TestDataFactory.createDelegatedContext({ conversationId: remainingConv.conversationId });
      mockAgentEngine.setMockContext(remainingConv.conversationId, context);
